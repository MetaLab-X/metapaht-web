...
<script>
	...
	function refreshPage(address, SendID)
	{
		window.parent.refreshPage2(address,SendID);
		parent.$.fancybox.close();
		//window.parent.location.reload();
	}
	function SettomainShop()
	{
		document.getElementById('sideshop').style.display = "none";
		document.getElementById('mainshop').style.display = "block";
		document.getElementById('setType').value="MainShop";
		
	}
	function SettosideShop()
	{
		document.getElementById('mainshop').style.display = "none";
		document.getElementById('sideshop').style.display = "block";
		document.getElementById('setType').value="SideShop";
	}
...
<input name="setType" id="setType" type="hidden" value="MainShop" />
...
<div class="col-xs-2"></div>
      <div class="col-xs-8" style="padding-top:15px;" align="right">
       <a class="btn btn-default btn-sm" href="#" style="margin-top:5px; background-color:#39F; color:#FFF; font-size:16px; border:0px;" onclick="return SettomainShop();">
 					 <i class="fa fa-user-plus " aria-hidden="true" style="font-size:13px;"></i> เพิ่มลูกค้าร้านหลัก
                 </a>
                   
                 <a class="btn btn-default btn-sm" href="#" style="margin-top:5px; background-color:#060; color:#FFF; font-size:16px; border:0px;" onclick="return SettosideShop();">
 					 <i class="fa fa-user-plus " aria-hidden="true" style="font-size:13px;"></i> เพิ่มลูกค้าร้านย่อย
                 </a>
      </div>
 <div class="col-xs-2"></div>
...
<div class="col-xs-2"></div>
      <div class="col-xs-8" style="padding-top:15px;">
          <div id="mainshop"><font style="font-size:18px;">ประเภทร้านค้า :</font>
          <select name="text0" id="text0" class="form-control" style="width:400px; height:40px;">
           <option value="1" selected="selected"> ร้านหลัก </option>
           <option value="3"> ร้านย่อยพิเศษ </option>
           <option value="4"> ร้านย่อยในพื้นที่ว่าง </option>
          </select>
      </div>
      
       <div id="sideshop" style="display:none;"><font style="font-size:18px;">ร้านค้าหลัก :</font>
           <select id="txt_1" name="txt_1" class="form-control" style="width:400px; height:40px;">
		<?php 
		for($i=1;$i<=$customerList[0][0];$i++){
		?>
    	<option value="<?=$customerList[$i][1];?>"><?=$customerList[$i][2];?></option>
    	<?php }?>
  </select>
      </div>
